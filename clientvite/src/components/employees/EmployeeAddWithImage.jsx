...
const handleSubmit = async (e) => {
  ...
  // Synchronously determine if there are errors from the *local* errors variable
  const isValid = Object.keys(errors).length === 0; 

  // Update the state with the new errors (this is asynchronous)
  setFormErrors(errors); 

  console.log('handleSubmit - form values are : ', formValues);

  // Check the *local* isValid variable to decide whether to submit
  if (isValid) { 
    setIsSubmit(true);
            // Prepare a standard JSON object (much cleaner)
      const postData = {
            ...
            // Include image URL if you have one, or handle image separately
            image: null, // Or the image URL if updating an existing record
            employee_salary: [
                {
    ...
    try {
      const response = await axiosInstance.post('/employees/', postData );
    ...
    console.log('handleSubmit - form errors are : ', errors); // Use the local 'errors' variable here
...
export default EmployeeAddWithImage